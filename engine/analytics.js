 ...
 * surveillance. See the citizen repo's docs/FUNCTIONAL_SPEC.md §7.1 and
 * §9-10 (https://github.com/hakrambuilders-cyber/biashara-guide.v2): this
 * module may only ever produce counts, percentages, and breakdowns — it
 * must never expose a single business's identity or raw profile. app.js
 * enforces that by design: it only ever renders the aggregates returned
 * here, never the underlying population.
 *
 * There is no real backend yet, so this module generates a synthetic
 * population and runs it through the exact same engine/core.js functions
 * that score a real citizen's profile — proving the aggregate view is
 * powered by the same brain, not a separate mocked-up model.
 * generateMockPopulation() takes a seed so the console can regenerate a
 * fresh-looking snapshot on demand (see app.js's flag-camouflaged reset
